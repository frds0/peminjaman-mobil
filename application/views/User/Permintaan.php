      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="card card-primary card-outline">
                ...
                <table id="example2" class="table table-bordered table-hover">
                  ...
                  <tbody>
                    <?php foreach ($permintaan as $row) {
                      if ($row->status == 'Dalam Pengajuan' || $row->status == 'Approved by Kadept' || $row->status == 'Tidak Disetujui oleh Kadept' || $row->status == 'Approved by Admin GA') { ?>
                        <tr>
                          ...
                          <?php if ($row->status == 'Dalam Pengajuan' || $row->status == 'Tidak Disetujui oleh Kadept') { ?>
                            <td align="center">
                              <a data-toggle="modal" data-target="#edit_permintaan<?php echo $row->id_permintaan; ?>" href="#edit_permintaan<?php echo $row->id_permintaan; ?>" class="btn btn-success"><i class="nav-icon fas fa-edit"></i></a>
                              <a data-toggle="modal" data-target="#detail_permintaan<?php echo $row->id_permintaan; ?>" href="#detail_permintaan<?php echo $row->id_permintaan; ?>" class="btn btn-primary" title="Detail Data"><i class="nav-icon fas fa-eye"></i></a>
                            </td>
                          <?php } ?>

                          <?php if ($row->status == 'Approved by Kadept') { ?>
                            <td align="center">
                              <a data-toggle="modal" data-target="#detail_permintaan<?php echo $row->id_permintaan; ?>" href="#detail_permintaan<?php echo $row->id_permintaan; ?>" class="btn btn-primary" title="Detail Data"><i class="nav-icon fas fa-eye"></i></a>
                            </td>
                          <?php } ?>
                          ...
                          <?php if ($row->status == 'Approved by Admin GA') { ?>
                            <td align="center">
                              <a data-toggle="modal" data-target="#start_user<?php echo $row->id_permintaan; ?>" href="#start_user<?php echo $row->id_permintaan; ?>" class="btn btn-warning" title="Start">Start</a>
                            </td>
                          <?php } ?>
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="card-primary">
                    ...
                    <!-- form start -->
                    <form method="post" action="<?php echo site_url('CTR_Permintaan/InsertData'); ?>" enctype="multipart/form-data">
                      <div class="card-body">
            ...
            <?php foreach ($permintaan as $row) {
              if ($row->status == 'Dalam Pengajuan' || $row->status == 'Approved by Kadept' || $row->status == 'Tidak Disetujui oleh Kadept' || $row->status == 'Approved by Admin GA') { ?>
                <div class="modal fade" id="edit_permintaan<?php echo $row->id_permintaan; ?>" tabindex="-1" aria-labelledby="edit_permintaan<?php echo $row->id_permintaan; ?>" aria-hidden="true">
                  <div class="modal-dialog">
                    <div class="modal-content">
                      <div class="card-primary">
                        <div class="card-header">
                        ...
                        <!-- form start -->
                        <form method="post" action="<?php echo site_url('CTR_Permintaan/EditData'); ?>" enctype="multipart/form-data">
                          <div class="card-body">
                            <div class="form-group" style="display: none;">
                              <label>ID Permintaan</label>
                              <input type="text" class="form-control" value="<?php echo $row->id_permintaan; ?>" name="id_permintaan">
                            </div>
                            <?php if ($row->status == 'Tidak Disetujui oleh Kadept') { ?>
                              <div class="form-group">
                                <label>Alasan Tidak Disetujui</label>
                                <textarea name="alasan_edit<?php echo $row->id_permintaan; ?>" class="form-control" readonly><?php echo $row->approved_tidaksetuju; ?></textarea>
                                <!-- <input type="text" style="height:100px" class="form-control" value="<?php echo $row->approved_tidaksetuju; ?>" name="alasan_edit<?php echo $row->id_permintaan; ?>" readonly> -->
                              </div>
                            <?php } ?>
            ...
            <?php foreach ($permintaan as $row) {
              if ($row->status == 'Dalam Pengajuan' || $row->status == 'Approved by Kadept' || $row->status == 'Tidak Disetujui oleh Kadept' || $row->status == 'Approved by Admin GA') { ?>
                <div class="modal fade" id="detail_permintaan<?php echo $row->id_permintaan; ?>" tabindex="-1" aria-labelledby="detail_permintaan<?php echo $row->id_permintaan; ?>" aria-hidden="true">
                      ...
                      <div class="modal-body">
                        ...
                        <div class="form-group row">
                          <div class="col-12">
                            ...
                            <!-- <dl class="row">
                              <?php if ($row->status == 'Tidak Disetujui oleh Kadept') { ?>
                                <dt class="col-sm-3">Alasan Tidak Disetujui</dt>
                              <?php } ?>
                            </dl> -->
                            <!-- <dl class="row">
                              <?php if ($row->status == 'Tidak Disetujui oleh Kadept') { ?>
                                <dd class="col-sm-12"><?php echo $row->approved_tidaksetuju; ?></dd>
                              <?php } ?>
                            </dl> -->
                          </div>
                        </div>
            ...
            <?php foreach ($permintaan as $row) {
              if ($row->status == 'Dalam Pengajuan' || $row->status == 'Approved by Kadept' || $row->status == 'Tidak Disetujui oleh Kadept' || $row->status == 'Approved by Admin GA') { ?>
                <div class="modal fade" id="start_user<?php echo $row->id_permintaan; ?>" tabindex="-1" aria-labelledby="start_user<?php echo $row->id_permintaan; ?>" aria-hidden="true">
                  <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                      <div class="modal-header bg-primary">
                        <h5 class="modal-title"><strong>Detail Data Permintaan</strong></h5>
                      ...
                      <form method="post" action="<?php echo site_url('CTR_Permintaan/start'); ?>" enctype="multipart/form-data">
                        <div class="form-group" style="display: none;">
                          <label>ID Permintaan</label>
                          <input type="text" class="form-control" value="<?php echo $row->id_permintaan; ?>" name="id_permintaan">
                        </div>
                        <div class="modal-body">
                          <div class="form-group row">
                            <div class="col-12">
                              ...
                              <dl class="row">
                                <dd class="col-sm-3" id="berangkat"><?php echo $row->berangkat; ?></dd>
                                <dd class="col-sm-3" id="kembali"><?php echo $row->kembali; ?></dd>
                                <dd class="col-sm-3" id="kembali"><?php echo $row->kendaraan; ?></dd>
                                <?php if ($row->kendaraan == 'Mobil') { ?>
                                  <dd class="col-sm-3" id="driver"><?php echo $row->driver; ?></dd>
                                <?php } ?>
                              </dl>
                              ...
                              <dl class="row">
                                <dd class="col-sm-3" id="kembali"><?php echo $row->mobil; ?></dd>
                                <?php if ($row->driver == 'Ya') { ?>
                                  <dd class="col-sm-3" id="namadriver"><?php echo $row->nama; ?></dd>
                                <?php } ?>
                                <?php if ($row->kendaraan == 'Mobil') { ?>
                                  <dd class="col-sm-3" id="emoney"><?php echo $row->tampilan_emoney; ?></dd>
                                <?php } ?>
                              </dl>
                            </div>
                          </div>
                          <div class="form-group row">
                            <div class="col-12">
                              ...
                              <div class="form-group">
                                <label>Alasan Assign Mobil</label>
                                <input type="text" class="form-control" value="<?php echo $row->alasan_assignmobil; ?>" name="edit_assignmobil<?php echo $row->id_permintaan; ?>" readonly>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div class="modal-footer">
                          <button type="button" class="btn btn-danger" data-dismiss="modal"><i class="fa fa-times"></i> Batal</button>
                          <button type="submit" class="btn btn-warning"><i class="fa fa-check"></i> Start</button>
                        </div>
                      </form>
                    </div>
                  </div>
