<?php
class MODEL_APermintaan extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT * FROM tbl_driver WHERE status = 'Aktif' AND available = 'Tersedia'");
		return $query->result();
	}

	function getdataemoney()
	{
		$query = $this->db->query("SELECT * FROM tbl_emoney WHERE status = 'Aktif' AND available = 'Tersedia'");
		return $query->result();
	}
}
